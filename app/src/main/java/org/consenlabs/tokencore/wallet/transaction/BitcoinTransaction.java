...
public class BitcoinTransaction implements TransactionSigner {
  ...
  public static class UTXO {
    private String txHash;//表示本次交易输入引用的是哪个交易的哈希值
    private int vout;//输出索引,表示引用的交易下的哪个UTXO,0代表第一个.
    private long amount;//解锁要花费的金额
    private String address;//收款人地址
    private String scriptPubKey;//解锁脚本/锁定脚本/脚本公钥/加密难题,作为输入,有发款人提供,作为输出由收款人提供
    private String derivedPath;//
    private long sequence = 4294967295L;//序列号 目前未被使用的交易替换功能,设为0xFFFF
  ...
  public TxSignResult signTransaction(String chainID, String password, Wallet wallet) {
    ...
    //add change output 找零
    long changeAmount = totalAmount - (getAmount() + getFee());
    if (changeAmount >= DUST_THRESHOLD) {
      tran.addOutput(Coin.valueOf(changeAmount), changeAddress);//changeAddress:即发款人自己的地址
    }
